...
int run(const cxxopts::ParseResult &opts) {
  ...
  try {
    khaiii_api->open(opts["rsc-dir"].as<string>(),
                     opts["opt-str"].as<string>());
  } catch (const khaiii::Except &exc) {
    _log->error("fail to open dir: '{}', opt: '{}'",
                opts["rsc-dir"].as<string>(), opts["opt-str"].as<string>());
    _log->error(exc.what());
    return 1;
  }
...
int main(int argc, char **argv) {
  ...
  options.add_options()("h,help", "print this help")(
      "rsc-dir", "resource directory",
      cxxopts::value<string>()->default_value(""))(
      "opt-str", "option (JSON format)",
      cxxopts::value<string>()->default_value(""))(
      ...
      cxxopts::value<string>()->default_value("all:info"));
