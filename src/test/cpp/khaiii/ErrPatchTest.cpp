...
public:
  ...
  virtual void TearDown() { ASSERT_NO_THROW(_khaiii_api->close()); }
...
protected:
  ...
  void _check(string raw, string left, string right) {
    ...
    if (left != bfr_str) {
      _log->warn("error not found: '{}' => E:'{}' vs A:'{}'", raw, left,
                 bfr_str);
      return;
    }
...
shared_ptr<spdlog::logger> ErrPatchTest::_log =
    spdlog::stderr_color_mt("ErrPatchTest");
